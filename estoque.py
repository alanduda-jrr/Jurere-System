...
# Proteção de acesso: se não estiver logado, redireciona ou avisa
...
def carregar_estoque():
  if os.path.exists(ARQUIVO_ESTOQUE):
    ...
    # Retorna DataFrame vazio com as colunas padrão se não existir
    return pd.DataFrame(
        columns=[
            "ID",
            "Produto",
...
st.markdown(
    "Cadastre novos produtos, consulte o estoque atual, edite valores ou"
    " remova itens."
)

# Abas para separar Cadastro, Consulta/Edição e Exclusão
aba1, aba2, aba3 = st.tabs(["➕ Cadastrar Item", "📋 Consultar / Editar", "🗑️ Excluir Item"])
  ...
  with st.form("form_cadastro", clear_on_submit=True):
    ...
    if submit_cadastro:
      if not nome_produto:
        ...
        # Define um ID único baseado no tamanho atual ou timestamp
