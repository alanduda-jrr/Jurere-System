...
LISTA_CATEGORIAS = [
    "Agua", "Refrigerantes", "Cervejas", "Drinks", 
    "Isotonicos e Eneréticos", "Porções", "Assados e Fritos", 
    "Sobremesas", "Snacks/Aperitivos"
]
...
def modal_editar(row_id):
  ...
  with st.form(key=f"form_modal_{row_id}"):
    ...
    if st.form_submit_button("💾 Salvar Alterações", use_container_width=True):
      ...
      salvar_estoque(df)
      st.session_state["msg_sucesso"] = f"✅ Alteração realizada com sucesso!"
      st.rerun()
...
col_b1, col_b2, _ = st.columns([1.5, 1.8, 6.7])
with col_b1:
  if st.button("➕ CADASTRAR", use_container_width=True, type="primary" if st.session_state["aba_ativa"] == "cadastro" else "secondary"):
    st.session_state["aba_ativa"] = "cadastro"
    st.rerun()
with col_b2:
  if st.button("📦 GALERIA", use_container_width=True, type="primary" if st.session_state["aba_ativa"] == "galeria" else "secondary"):
    st.session_state["aba_ativa"] = "galeria"
    st.rerun()
...
if st.session_state["aba_ativa"] == "cadastro":
  ...
  with st.form("form_cadastro_avancado", clear_on_submit=True):
    ...
    margem_calc = ((preco_venda - preco_custo) / preco_venda * 100) if preco_custo > 0 and preco_venda > 0 else 0.0
    st.markdown(f"ℹ️ **Margem de Lucro Estimada:** `{margem_calc:.2f}%`")
    
    if st.form_submit_button("Salvar Produto no Estoque", use_container_width=True):
      if not nome_produto or preco_venda <= 0:
        ...
        novo_dado = pd.DataFrame([{
            "ID": novo_id, "Produto": nome_produto, "Categoria": categoria,
            "Preço de Custo (R$)": preco_custo, "Preço de Venda (R$)": preco_venda,
            "Margem (%)": round(margem_calc, 2), "Quantidade": quantidade,
            "Estoque Mínimo": estoque_minimo, "Imagem": caminho_imagem,
        }])
        salvar_estoque(pd.concat([df_estoque, novo_dado], ignore_index=True))
  ...
  if df_estoque.empty or df_estoque["Produto"].dropna().empty:
    ...
    with col_f1:
      termo_busca = st.text_input("🔍 Pesquisa:", placeholder="Filtrar...", label_visibility="collapsed")
    with col_f2:
      cat_filtro = st.selectbox("Categoria", ["Todas as categorias"] + list(df_estoque["Categoria"].dropna().unique()), label_visibility="collapsed")
          ...
          with st.container(border=True):
            ...
            if not df_caixa.empty and "Descricao" in df_caixa.columns:
              for _, v_row in df_caixa.iterrows():
                if produto_nome.lower() in str(v_row["Descricao"]).lower():
                  vendas_semana += 1
            ...
            <div style="font-size: 11px; line-height: 1.3; margin-bottom: 2px; text-align: center;">
              ...
              <div style="background-color: {bg_estoque}; color: {cor_estoque}; padding: 3px 6px; border-radius: 4px; margin-bottom: 3px; font-weight: bold;">📦 Estoque: {qtd} un.{alerta_txt}</div>
              <div style="background-color: #e2f0d9; color: #276a16; padding: 3px 6px; border-radius: 4px; margin-bottom: 6px; font-weight: bold;">📈 Vendas (7d): {vendas_semana} un.</div>
