...
# Funções de carregamento seguras com tratamento de dados vazios
...
if df_estoque.empty or "Produto" not in df_estoque.columns or df_estoque["Produto"].dropna().empty:
  st.info("⚠️ Nenhum produto cadastrado no estoque para iniciar as vendas. Cadastre itens na aba de estoque primeiro.")
else:
  # Interface básica do PDV com baixa integrada
  ...
  with col1:
    ...
    if st.button("Finalizar Venda e Dar Baixa", type="primary", use_container_width=True):
      if estoque_atual < quantidade_comprar:
        ...
        # 1. Atualiza o estoque subtraindo a quantidade
        ...
        # 2. Registra no caixa para computar as vendas dos últimos 7 dias
        ...
        st.success(f"✅ Venda de {descricao_venda} realizada com sucesso! Estoque atualizado.")
        st.rerun()
